""" This scripts generates 3 examples of low resolution (LR) synthetic T1 scans along with corresponding high resolution
(HR) real scans (regression targets).
...
# general parameters
channels = 1  # we only generate 1 synthetic channel
output_channel = None  # the regression targets are not synthetic, but real
target_res = None  # we want to produce synthetic data at the same resolution as the input label maps
output_shape = None  # in this example we do not randomly crop the generated pairs
...
prior_means = '../../data/labels_classes_priors/prior_means_t1_lr.npy'
# same as for prior_means, but for the standard deviations of the GMM.
prior_stds = '../../data/labels_classes_priors/prior_stds_t1_lr.npy'
...
# simulate, we decide here to downsample the Gaussian image to LR.
downsample = True
